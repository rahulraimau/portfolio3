  ...
  return (
    <section id="skills" className="py-20 px-6 bg-gray-100 text-gray-900 text-center">
      ...
      <div className="mb-8 flex flex-wrap justify-center gap-3">
        {tags.map((tag) => (
          <span key={tag} className="bg-gray-800 text-white px-3 py-1 rounded-full text-sm font-medium">
            #{tag}
          </span>
        ))}
      </div>
      <div className="flex flex-wrap justify-center gap-4 max-w-5xl mx-auto">
        {skills.map((skill) => (
          <span key={skill} className="bg-yellow-300 text-black px-4 py-2 rounded-full font-medium hover:bg-yellow-400 transition">
            {skill}
          </span>
        ))}
      </div>
